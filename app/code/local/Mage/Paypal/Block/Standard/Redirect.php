...
class Mage_Paypal_Block_Standard_Redirect extends Mage_Core_Block_Abstract
{
    protected function _toHtml()
    {
        ...
        foreach ($standard->getStandardCheckoutFormFields() as $field=>$value) {
              if($field == 'amount_1'):                
                $price = $value/85;               
                $form->addField($field, 'hidden', array('name'=>$field, 'value'=>$price));
        ...
        $submitButton = new Varien_Data_Form_Element_Submit(array(
            'value'    => $this->__('Click here if you are not redirected within 105 seconds...'),
        ));
        $id = "submit_to_paypal_button_{$idSuffix}";
        $submitButton->setId($id);
        $form->addElement($submitButton);
        $html = '<html><body>';
        $html.= $this->__('You will be redirected to the PayPal website in a few4 seconds.');
        $html.= $form->toHtml();
