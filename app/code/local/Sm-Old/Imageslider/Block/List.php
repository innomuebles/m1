...
class Sm_Imageslider_Block_List extends Mage_Catalog_Block_Product_Abstract
{
	...
	const CACHE_TAG        = 'SM_IMAGESLIDER';
	public function __construct($attr){
		parent::__construct($attr);
		$this->addData(array(
            'cache_lifetime' => 999999,
            'cache_tags'     => array(
									Sm_Imageslider_Block_List::CACHE_TAG,
	...
	public function _getCfg($attr = null)
	{
		...
		$def_cfgs = Mage::getConfig()
									...
									->getNode('default/imageslider_cfg')->asArray(); 
		...
		if(!empty($items)) {
			foreach($items as $item){
				...
				if( $item['image'] != '' ){
					...
					//$item['link'] = $item['link'];
					$item['_image'] = (strpos($item['image'],'http') !== false) ? $item['image'] : Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA).''.$item['image'];
