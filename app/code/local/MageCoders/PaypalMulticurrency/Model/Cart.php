 ...
 */
class MageCoders_PaypalMulticurrency_Model_Cart extends Mage_Paypal_Model_Cart
{
    /**
    ...
    public function updateTotal($code, $amount, $lineItemDescription = null)
    {
        ...
        if (isset($this->_totals[$code])) {
            $this->_totals[$code] += $this->_helper->convertCurrency($amount);
    ...
    protected function _render()
    {
        ...
        if ($this->_salesEntity instanceof Mage_Sales_Model_Order) {
			...
			$this->_totals = array(
				self::TOTAL_SUBTOTAL => $this->_prepareSubtotal($this->_salesEntity->getSubtotal()),
                self::TOTAL_TAX      => $this->_helper->convertCurrency($this->_salesEntity->getTaxAmount()),
                self::TOTAL_SHIPPING => $this->_helper->convertCurrency($this->_salesEntity->getShippingAmount()),
                self::TOTAL_DISCOUNT => $this->_helper->convertCurrency(abs($this->_salesEntity->getDiscountAmount())),
            );
            ...
            $this->_totals = array (
                self::TOTAL_SUBTOTAL => $this->_prepareSubtotal($this->_salesEntity->getSubtotal()),
                self::TOTAL_TAX      => $this->_helper->convertCurrency($address->getTaxAmount()),
                self::TOTAL_SHIPPING => $this->_helper->convertCurrency($address->getShippingAmount()),
                self::TOTAL_DISCOUNT => $this->_helper->convertCurrency(abs($address->getDiscountAmount())),
            );
        ...
        if ($originalDiscount > 0.0001 && isset($this->_totalLineItemDescriptions[self::TOTAL_DISCOUNT])) {
			...
			$originalDiscount = $this->_helper->convertCurrency($originalDiscount);
    ...
    protected function _validate()
    {
      ...
      //  $referenceAmount = $this->_salesEntity->getBaseGrandTotal();
	    $referenceAmount = $this->_helper->convertCurrency($this->_salesEntity->getGrandTotal());
        ...
        foreach ($this->_items as $i) {
			if(is_object($i)){
				//$itemsSubtotal = $itemsSubtotal + $i->getQty() * $this->_helper->convertCurrency($i->getAmount());
				$itemsSubtotal = $itemsSubtotal + $i->getQty() * $i->getAmount();
			}else{
				//$itemsSubtotal = $itemsSubtotal + $i['qty'] * $this->_helper->convertCurrency($i['amount']);
        ...
        }elseif(floor($sum) == floor($referenceAmount)){
	            $this->_areItemsValid = true;
		}
    ...
    protected function _addRegularItem(Varien_Object $salesItem)
    {
        if ($this->_salesEntity instanceof Mage_Sales_Model_Order) {
           ...
           // $amount = (float) $salesItem->getBasePrice();
		    $amount = (float) $this->_helper->convertCurrency($salesItem->getPrice());
            // TODO: nominal item for order
        } else {
            $qty = (int) $salesItem->getTotalQty();
           // $amount = $salesItem->isNominal() ? 0 : (float) $salesItem->getBaseCalculationPrice();
		    $amount = $salesItem->isNominal() ? 0 : (float) $this->_helper->convertCurrency($salesItem->getCalculationPrice());
        }
        ...
        if (($amount * $qty) != $this->_helper->convertCurrency($salesItem->getRowTotal())) {
            $amount = (float) $this->_helper->convertCurrency($salesItem->getRowTotal());
            $subAggregatedLabel = ' x' . $qty;
            $qty = 1;
        }
    ...
    {
       // $this->_totals[self::TOTAL_TAX] += (float)$this->_helper->convertCurrency($salesEntity->getBaseHiddenTaxAmount());
	   
$this->_totals[self::TOTAL_TAX] += (float)$this->_helper->convertCurrency($salesEntity->getHiddenTaxAmount());	   
       // $this->_totals[self::TOTAL_TAX] += (float)$this->_helper->convertCurrency($salesEntity->getBaseShippingHiddenTaxAmount());
$this->_totals[self::TOTAL_TAX] += (float)$this->_helper->convertCurrency($salesEntity->getShippingHiddenTaxAmount());	   
    }
	
	
	protected function _prepareSubtotal($subtotal){
	
		$subtotal = $this->_helper->convertCurrency($subtotal);
		...
		if (sprintf('%.4F', $itemsSubtotal) != sprintf('%.4F', $subtotal)){
