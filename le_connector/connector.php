...
<head>
    <title>Connector is successfully Installed!</title>
...
<body style="">
<div ">
    <div style="margin-left: calc(50% - 28px)">
        <svg class="checkmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 52 52"><circle class="checkmark__circle" cx="26" cy="26" r="25" fill="none"/><path class="checkmark__check" fill="none" d="M14.1 27.2l7.1 7.2 16.7-16.8"/></svg>
    </div>
    <div class="" style="margin-top: 20px;font-size: 18px;font-family: Arial;">Connector is successfully installed!</div>
</div>
</body>
</html>
';
    ...
    function run(){
        ...
        if($query){
            ...
            if($processQuery['result']){
                $res = json_encode($processQuery['data']);
            }else{
                $res = $processQuery['msg'];
            }
        }
        $this->render($query,$res);
    }
    function render($query, $res){
        if($res){

            $res = '<div style="width: 100%;height: auto; padding: 10px;margin-top: 10px;border: 1px solid" >
    <p>'.$res.'</p>
    </div>';
        }
        ...
        <textarea name="query" id="" cols="30" rows="10" style="width: 100%;height: auto;padding: 10px" ">'.$query.'</textarea>
            <input type="submit" name="" style="width: 75px;height: 25px; padding: 5px; margin: 10px 0px" value="execute">'.$res.'
        </div>
    </form>';
    }
}
